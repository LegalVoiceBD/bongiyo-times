...
export default async function Home({ searchParams }: { searchParams: { category?: string, tab?: string, page?: string, q?: string } }) {
  ...
  // New Grid Section Data (below hero)
  const gridSectionNews = allNews.slice(10, 24);
  ...
  // Adjusted fallback indices to avoid overlaps with gridSectionNews
  const bdNews = getCategoryNews('বাংলাদেশ', 25, 4);
  const lifestyleNews = getCategoryNews('জীবনযাপন', 30, 4);
  const entertainmentNews = getCategoryNews('বিনোদন', 35, 3); 
  const politicsNews = getCategoryNews('রাজনীতি', 40, 3); 
  const eduNews = getCategoryNews('শিক্ষা', 45, 2);
  const jobsNews = getCategoryNews('চাকরি', 50, 2);
  const techNews = getCategoryNews('প্রযুক্তি', 55, 2);
  const businessNews = getCategoryNews('বাণিজ্য', 60, 2); 
      ...
      <main className="max-w-[1200px] mx-auto px-4 mt-6 pb-10">
        ...
        {(activeCategory || searchQuery) ? (
          ...
          <>
            ...
            {/* NEW GRID SECTION (Thin Border Style below Hero) */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-y-6 lg:gap-x-0 mb-8 border-b border-gray-300 pb-8 divide-y lg:divide-y-0 lg:divide-x divide-gray-300">
               
               {/* Left Column */}
               <div className="lg:col-span-3 flex flex-col divide-y divide-gray-300 lg:pr-6">
                  {gridSectionNews.slice(0, 5).map((news, idx) => (
                     <a href={news.is_custom ? `/news/${news.id}` : news.source_url} target="_blank" key={news.id} className={`group block ${idx !== 0 ? 'pt-4' : ''} pb-4`}>
                        {idx === 0 ? (
                           <div className="flex gap-3">
                              <div className="flex-1">
                                 <h3 className="text-[16px] font-bold text-[#1a1a1a] group-hover:text-blue-600 leading-snug">{news.title}</h3>
                                 <p className="text-[12px] text-gray-500 mt-2">{formatDateTime(news.created_at)}</p>
                              </div>
                              <SafeImage src={news.image_url} alt={news.title} className="w-[80px] h-[60px] object-cover rounded-sm" />
                           </div>
                        ) : (
                           <>
                              <h3 className="text-[16px] font-bold text-[#1a1a1a] group-hover:text-blue-600 leading-snug">{news.title}</h3>
               ...
               {/* Middle Column (Grid Style) */}
               <div className="lg:col-span-6 lg:px-6 flex flex-col pt-6 lg:pt-0">
                  {/* Row 1 (Images) */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-300 border-b border-gray-300 pb-4 mb-4">
                     {gridSectionNews.slice(5, 7).map((news, idx) => (
                        <a href={news.is_custom ? `/news/${news.id}` : news.source_url} target="_blank" key={news.id} className={`group block ${idx === 0 ? 'sm:pr-4 pb-4 sm:pb-0' : 'sm:pl-4 pt-4 sm:pt-0'}`}>
                           <SafeImage src={news.image_url} alt={news.title} className="w-full h-[140px] object-cover mb-2 rounded-sm" />
                           <h3 className="text-[17px] font-bold text-[#1a1a1a] group-hover:text-blue-600 leading-snug">{news.title}</h3>
                           <p className="text-[12px] text-gray-500 mt-2">{formatDateTime(news.created_at)}</p>
                        </a>
                     ))}
                  </div>
                  {/* Row 2 (Text with excerpts) */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-300 border-b border-gray-300 pb-4 mb-4">
                     {gridSectionNews.slice(7, 9).map((news, idx) => (
                        <a href={news.is_custom ? `/news/${news.id}` : news.source_url} target="_blank" key={news.id} className={`group block ${idx === 0 ? 'sm:pr-4 pb-4 sm:pb-0' : 'sm:pl-4 pt-4 sm:pt-0'}`}>
                           <h3 className="text-[17px] font-bold text-[#1a1a1a] group-hover:text-blue-600 leading-snug">{news.title}</h3>
                           <p className="text-[14px] text-gray-600 mt-2 line-clamp-2">সংবাদটির বিস্তারিত জানতে লিংকে ক্লিক করে সম্পূর্ণ খবরটি পড়ুন।</p>
                           <p className="text-[12px] text-gray-500 mt-2">{formatDateTime(news.created_at)}</p>
                        </a>
                     ))}
                  </div>
                  {/* Row 3 (Text only) */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-300 pb-4 lg:pb-0">
                     {gridSectionNews.slice(9, 11).map((news, idx) => (
                        <a href={news.is_custom ? `/news/${news.id}` : news.source_url} target="_blank" key={news.id} className={`group block ${idx === 0 ? 'sm:pr-4 pb-4 sm:pb-0' : 'sm:pl-4 pt-4 sm:pt-0'}`}>
                           <h3 className="text-[16px] font-bold text-[#1a1a1a] group-hover:text-blue-600 leading-snug">{news.title}</h3>
                           <p className="text-[12px] text-gray-500 mt-2">{formatDateTime(news.created_at)}</p>
                        </a>
               ...
               {/* Right Column */}
               <div className="lg:col-span-3 flex flex-col divide-y divide-gray-300 lg:pl-6 pt-6 lg:pt-0">
                  {gridSectionNews.slice(11, 14).map((news, idx) => (
                     <a href={news.is_custom ? `/news/${news.id}` : news.source_url} target="_blank" key={news.id} className={`group block ${idx !== 0 ? 'pt-4' : ''} pb-4`}>
                        <p className="text-[13px] font-bold text-red-600 mb-1">{news.category} •</p>
                        <div className="flex gap-3">
                           <div className="flex-1">
                              <h3 className="text-[16px] font-bold text-[#1a1a1a] group-hover:text-blue-600 leading-snug">{news.title}</h3>
                              <p className="text-[12px] text-gray-500 mt-2">{formatDateTime(news.created_at)}</p>
                           </div>
                           {(idx === 1 || idx === 2) && <SafeImage src={news.image_url} alt={news.title} className="w-[80px] h-[60px] object-cover rounded-sm" />}
                        </div>
                     </a>
                  ))}
               </div>
            ...
            {/* CATEGORY LAYOUT 3: বিনোদন & রাজনীতি */}
            ...
            {/* CATEGORY LAYOUT 4: শিক্ষা, চাকরি, প্রযুক্তি, বাণিজ্য */}
